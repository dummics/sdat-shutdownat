...
Interactive CLI results use centered Spectre.Console panels for schedules, previews, status, cancellation, and errors. When a command is launched from Win+R, the transient console keeps the result visible for six seconds; commands run inside an existing terminal return immediately. Redirected and `--json` output remain automation-safe.
...
Search for **ShutdownAT** in Start, run `sdat ui`, or launch `SDAT.exe` directly. The executable keeps its compact technical name while the installed product is presented as ShutdownAT. Its WinUI 3 shell keeps Overview and Schedule in the main navigation and places notifications, language, quick access, diagnostics, logging, developer tools, and product information under one scrollable Settings surface with always-visible sections and a fixed Save action. Static and dynamic UI text is localized in English and Italian using Windows MRT Core resources. The app follows the Windows language by default, or you can choose Italian or English in Settings and apply the change with a one-click restart.

Schedule inputs are previewed without changing SQLite or Task Scheduler, using the same parser as the CLI and TUI. The Schedule page and quick palette resolve relative durations and clock times before the primary action is enabled. Overview shows the next execution and offers only relevant actions, while the tray derives its labels and controls from current authoritative state instead of maintaining a separate schedule copy.
...
The configurable global hotkey (default `Ctrl+Alt+S`) toggles the compact quick scheduler whenever ShutdownAT is running. The palette also closes with Escape or by clicking another window. Enabling startup keeps one per-user companion instance available in the notification area after sign-in; the installed app repairs a missing or stale Windows startup entry from that saved preference whenever it is opened. Closing the main window then hides it without stopping the tray or hotkey. Opening ShutdownAT again reuses that background instance. A conflicting hotkey does not take down the app or tray; SDAT reports the conflict and keeps the previous working combination when possible.

Reminder timing, countdown placement, quick-palette placement, startup behavior, overlap policy, and hotkey are local settings. The compact critical countdown defaults to the top center; the quick palette defaults to the bottom center. Both support every screen edge and corner and switch to a vertical action layout on the left or right edge. Palette feedback expands toward the free screen space without moving the command row, and an active one-time schedule can be cancelled from a control that identifies its action and time. In-app status messages stay aligned with the content, can be dismissed, and close automatically; synthetic test notifications also use Windows' short transient duration. Reminder actions carry the schedule id and revision, so an old notification cannot mutate a newer SDAT schedule. Every explicit Cancel still attempts the fail-safe native Windows countdown abort. Successful cancellation publishes a local revision-safe signal and the critical overlay also watches authoritative SQLite state, so cancellation from a toast, the overlay, tray, palette, TUI, or `sdat cancel` closes matching open countdown surfaces across processes. Dismiss only closes the reminder; cancelling stops the matching schedule, and Snooze is available for one-time actions.
...
Developer mode is available as a clearly labeled Settings section. Enabling it reveals safe notification, countdown, and quick-palette previews. Safe test mode blocks new real schedules before they reach SQLite or Task Scheduler, pauses migration and reconciliation, and ignores already-projected task invocations before ledger, notification, finalization, or Windows power-action work. Its previews use synthetic data and cannot cancel, snooze, or promote a test into a real schedule.

Windows notification availability and Focus/Do Not Disturb behavior remain controlled by Windows. SDAT does not claim an unconditional bypass. For shutdown and restart, Windows still owns the final 30-second system countdown; SDAT mirrors it in a compact on-screen overlay whose cancel action aborts that native countdown directly.
