...
if "%SDAT_FROM_WINR%"=="1" (
  echo %* | findstr /i /c:"-SelfTest" >nul && (
    start "SDAT SelfTest" "%PS_EXE%" -NoProfile -ExecutionPolicy Bypass -File "%SDAT_DIR%\tools\selftest-window.ps1" %*
    exit /b 0
  )
)

"%PS_EXE%" -ExecutionPolicy Bypass -File "%SDAT_DIR%\shutdownat.ps1" %*
