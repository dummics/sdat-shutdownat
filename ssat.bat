...
if "%SDAT_FROM_WINR%"=="1" (
  echo %* | findstr /i /c:"-SelfTest" >nul && (
    start "SDAT SelfTest" powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SDAT_DIR%\tools\selftest-window.ps1" -Suspend %*
    exit /b 0
  )
)

powershell -ExecutionPolicy Bypass -File "%SDAT_DIR%\shutdownat.ps1" -Suspend %*
